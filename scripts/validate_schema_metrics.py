...
Batch 1: with empty ALL_METRICS the CLI still exercises commits_export populate via the
runner; metric modules register in later PRs.
"""
...
def validation_failure_footer() -> str:
    """Short footer printed after a validation failure (metric modules may override later)."""
    return (
        "See schema/metrics_conventions.md and docs/adr/0001-minimal-commit-storage-schema.md."
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            f"Metrics: {METRIC_ALL} (SQL parity: {', '.join(ALL_METRICS) or '(none registered yet)'}), "
            f"opt-in: {', '.join(OPT_IN_METRICS) or '(none)'}. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"{', '.join(RUNNABLE_METRICS) or '(no metrics registered yet)'}."
        ),
    )
    parser.add_argument(
        "--out-dir",
        metavar="DIR",
        help="Reserved for later metric PRs (e.g. cycle_time_monthly canonical CSVs)",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
