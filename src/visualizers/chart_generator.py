...
def save_metrics_data(cycle_time_data=None, failure_rate_data=None, prefix=None):
    ...
    if cycle_time_data:
        df = pd.DataFrame(
            cycle_time_data, columns=["Month", "Sum", "Average", "p75", "std"]
        )
        df.to_csv(
            os.path.join(metrics_dir, f"{prefix}cycle_time_data.csv"), index=False
        )

    if failure_rate_data:
        df = pd.DataFrame(failure_rate_data, columns=["Month", "Rate"])
        df.to_csv(
            os.path.join(metrics_dir, f"{prefix}change_failure_data.csv"), index=False
        )
...
def prepare_cycle_time_chart_data(cycle_time_data):
    """
    Isolate data prep for cycle time chart. Returns DataFrame ready for plotting.

    Args:
        cycle_time_data: List of tuples (month, sum, average, p75, std)
    Returns:
        pd.DataFrame with Month (datetime), p75 and std in days
    """
    df = pd.DataFrame(
        cycle_time_data, columns=["Month", "Sum", "Average", "p75", "std"]
    )
...
def prepare_change_failure_chart_data(failure_rate_data):
    """
    Isolate data prep for change failure rate chart. Returns DataFrame ready for plotting.

    Args:
        failure_rate_data: List of tuples (month, rate)
    Returns:
        pd.DataFrame with Month (datetime) and Rate, sorted by Month
    """
    df = pd.DataFrame(failure_rate_data, columns=["Month", "Rate"])
    df["Month"] = pd.to_datetime(df["Month"], format="%Y-%m")
...
def plot_cycle_time(
    ...
    Args:
        cycle_time_data: List of tuples (month, sum, average, p75, std)
        output_file: Name of the output file (used when output_path is None)
        output_path: Optional full path for the output file (used by tests for tmp_path)
    """
    setup_plot_style()
...
def _render_cycle_time_chart(df, output_file, output_path):
    ...
    plt.plot(
        df["Month"], trendline, "--", label="Trendline", alpha=0.7, color="#2B5F8E"
    )  # Darker blue

    # Add trendline value annotation
    last_value = trendline[-1]
    plt.annotate(
        f"Trend: {last_value:.1f} days",
...
def _render_change_failure_chart(df, output_file, output_path):
    ...
    plt.axhline(
        y=10, color="#666666", linestyle=":", alpha=0.5, label="Industry Standard (10%)"
    )
...
def plot_work_categories(client_name, csv_filename=None, title=None):
    ...
    plt.title(
        title
        or f"Work Categories Distribution - {client_name.replace('_', ' ').title()}",
        pad=20,
    )
    plt.xlabel("Month")
