...
from git_calculator.git_ir import git_log, format_git_logs_as_string
from git_calculator.visualizers.chart_generator import setup_plot_style, ensure_metrics_dir
...
def calculate_percentiles(
    commits_by_author: Dict[str, List[Tuple[datetime, int]]],
) -> Dict[str, float]:
    """
    ...
    percentiles = {
        author: (rank / len(commit_values)) * 100
        for author, rank in zip(total_commits.keys(), ranks)
    }
    return percentiles


def plot_commit_trends(
    commits_by_author: Dict[str, List[Tuple[datetime, int]]],
    output_file: str = "commit_trends.png",
):
    """
    ...
    for author, commits in commits_by_author.items():
        for date, count in commits:
            all_data.append({"Date": date, "Commits": count, "Author": author})
...
def save_commit_data(
    commits_by_author: Dict[str, List[Tuple[datetime, int]]], prefix: str = "commit_"
):
    """
    ...
    for author, commits in commits_by_author.items():
        ...
        df.to_csv(
            os.path.join(metrics_dir, f"{prefix}{author}_commits.csv"), index=False
        )

    # Save percentile data
    percentiles = calculate_percentiles(commits_by_author)
    df_percentiles = pd.DataFrame(
        {"Author": list(percentiles.keys()), "Percentile": list(percentiles.values())}
    )
    df_percentiles.to_csv(
        os.path.join(metrics_dir, f"{prefix}percentiles.csv"), index=False
    )
...
def analyze_commits():
    ...
    try:
        ...
        # Print percentiles
        percentiles = calculate_percentiles(commits_by_author)
        for author, percentile in sorted(
