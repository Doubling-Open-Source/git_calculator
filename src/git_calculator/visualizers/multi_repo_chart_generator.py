...
from git_calculator.visualizers.chart_generator import setup_plot_style
...
class MultiRepoChartGenerator:
    ...
    def plot_cycle_time_comparison(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "cycle_time_comparison.png",
    ) -> str:
        """
        ...
        for repo_name, repo_metrics in metrics.items():
            ...
            df = pd.DataFrame(
                cycle_data, columns=["Month", "Sum", "Average", "P75", "Std"]
            )
            ...
            if len(df) > 1:
                ...
                plt.plot(
                    df["Month"], trendline, "--", alpha=0.6, color=color, linewidth=1
                )
        ...
        plt.title(
            "Cycle Time Comparison Across Repositories (P75)", pad=20, fontsize=14
        )
    ...
    def plot_failure_rate_comparison(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "failure_rate_comparison.png",
    ) -> str:
        """
        ...
        plt.title(
            "Change Failure Rate Comparison Across Repositories", pad=20, fontsize=14
        )
        plt.xlabel("Month", fontsize=12)
    ...
    def plot_active_developers_comparison(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "active_developers_comparison.png",
    ) -> str:
        """
        ...
        plt.title(
            "Active Developers Comparison Across Repositories", pad=20, fontsize=14
        )
        plt.xlabel("Month", fontsize=12)
    ...
    def plot_repository_summary(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "repository_summary.png",
    ) -> str:
        """
        ...
        for repo_name, repo_metrics in metrics.items():
            ...
            if cycle_data:
                avg_cycle_time = (
                    sum(point[3] for point in cycle_data) / len(cycle_data) / (24 * 60)
                )  # Convert to days
            else:
                avg_cycle_time = 0
            ...
            failure_data = repo_metrics.get("failure_rate_data", [])
            if failure_data:
                avg_failure_rate = sum(point[1] for point in failure_data) / len(
                    failure_data
                )
            else:
                avg_failure_rate = 0
        ...
        bars3 = ax3.bar(
            repo_names, avg_cycle_times, color=self.colors[: len(repo_names)]
        )
        ...
        bars4 = ax4.bar(
            repo_names, avg_failure_rates, color=self.colors[: len(repo_names)]
        )
    ...
    def plot_throughput_per_active_dev_comparison(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "throughput_per_active_dev_comparison.png",
    ) -> str:
        """
        ...
        for i, (repo_name, repo_metrics) in enumerate(metrics.items()):
            throughput_per_active_dev_data = repo_metrics.get(
        ...
        ax.set_title(
            "Throughput Per Active Developer Comparison Across Repositories (Weekly)",
            pad=20,
            fontsize=14,
        )
        ...
        logger.info(
            f"Generated throughput per active developer comparison chart: {filepath}"
        )
        return filepath

    def plot_throughput_per_active_dev_stacked_bar(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "throughput_per_active_dev_stacked_bar.png",
    ) -> str:
        """
        ...
        for repo_name, repo_metrics in metrics.items():
            ...
            repo_data[repo_name] = {"weeks": repo_weeks, "throughput": repo_throughput}

        if not all_weeks:
            logger.warning(
                "No throughput per active developer data found for stacked bar chart"
            )
            return ""
        ...
        ax.set_title(
            "Throughput Per Active Developer - Stacked Bar Chart (Weekly)",
            pad=20,
            fontsize=14,
        )
        ax.set_xlabel("Week", fontsize=12)
        ...
        logger.info(
            f"Generated throughput per active developer stacked bar chart: {filepath}"
        )
        return filepath

    def plot_throughput_per_active_dev_combined(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "throughput_per_active_dev_combined.png",
    ) -> str:
        """
        ...
        bars = ax.bar(
            ...
            color="darkgreen",
            alpha=0.7,
            label="Throughput Per Active Developer",
        )

        ax.set_title(
            "Throughput Per Active Developer by Week (Combined Across All Repositories)",
            pad=20,
            fontsize=14,
        )
        ax.set_xlabel("Week", fontsize=12)
        ...
        logger.info(
            f"Generated throughput per active developer combined chart: {filepath}"
        )
        return filepath

    def plot_active_developers_weekly_comparison(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "active_developers_weekly_comparison.png",
    ) -> str:
        """
        ...
        ax.set_title(
            "Active Developers by Week Comparison Across Repositories",
            pad=20,
            fontsize=14,
        )
        ax.set_xlabel("Week", fontsize=12)
    ...
    def plot_active_developers_weekly_combined(
        self,
        metrics: Dict[str, Dict[str, Any]],
        filename: str = "active_developers_weekly_combined.png",
    ) -> str:
        """
        ...
        bars = ax.bar(
            ...
            color="steelblue",
            alpha=0.7,
            label="Unique Active Developers",
        )

        ax.set_title(
            "Unique Active Developers by Week (Combined Across All Repositories)",
            pad=20,
            fontsize=14,
        )
        ax.set_xlabel("Week", fontsize=12)
    ...
    def generate_all_comparison_charts(
        ...
        try:
            ...
            logger.error(
                f"Failed to generate throughput per active developer combined chart: {e}"
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to generate active developers weekly combined chart: {e}"
            )
        ...
        logger.info(
            f"Generated {len(generated_charts)} comparison charts in {self.output_dir}"
        )
