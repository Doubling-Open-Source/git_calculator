...
class MultiRepoCalculator:
    ...
    def calculate_repo_metrics(self, repo_name: str) -> Dict[str, Any]:
        ...
        try:
            with self.repo_manager.repository_context(repo_name):
                ...
                cycle_time_data = cycle_calc.commit_statistics_normalized_by_month(
                    time_deltas
                )
                ...
                active_dev_data = [
                    (month, authors_set, len(authors_set))
                    for month, authors_set in authors_by_month.items()
                ]

                # Calculate throughput
                commits_and_authors = tc.extract_commits_and_authors(logs)
                throughput_data = [
                    (month, authors_set, commit_count)
                    for month, (
                        authors_set,
                        commit_count,
                    ) in commits_and_authors.items()
                ]

                # Calculate throughput per active developer (past 4 weeks) - weekly granularity
                normalized_throughput_data = (
                    tc.calculate_throughput_per_active_developer_by_week(
                        logs, weeks_back=4
                    )
                )
                throughput_per_active_dev_data = [
                    (week, commits, active_dev_count, throughput_per_dev)
                    for week, (
                        commits,
                        active_dev_count,
                        throughput_per_dev,
                    ) in normalized_throughput_data.items()
                ]

                # Calculate active developers by week (past 4 weeks)
                active_dev_by_week_data = tc.calculate_active_developers_by_week(
                    logs, weeks_back=4
                )
                active_dev_weekly_data = [
                    (week, commits, active_dev_count, active_dev_emails)
                    for week, (
                        commits,
                        active_dev_count,
                        active_dev_emails,
                    ) in active_dev_by_week_data.items()
                ]
                ...
                logger.info(
                    f"Successfully calculated metrics for repository '{repo_name}'"
                )
                return metrics

        except Exception as e:
            logger.error(
                f"Failed to calculate metrics for repository '{repo_name}': {e}"
            )
            return {}
    ...
    def aggregate_cycle_time_metrics(
    ...
    def aggregate_failure_rate_metrics(
        self, metrics: Dict[str, Dict[str, Any]]
    ...
    def aggregate_active_developers_metrics(
        self, metrics: Dict[str, Dict[str, Any]]
    ) -> List[Tuple[str, int]]:
        """
    ...
    def aggregate_throughput_metrics(
        self, metrics: Dict[str, Dict[str, Any]]
    ) -> List[Tuple[str, int]]:
        """
    ...
    def aggregate_throughput_per_active_dev_metrics(
        self, metrics: Dict[str, Dict[str, Any]]
    ) -> List[Tuple[str, float]]:
        """
        ...
        weekly_data = defaultdict(
            lambda: {"total_commits": 0, "unique_active_devs": set()}
        )
        ...
        for repo_name, repo_metrics in metrics.items():
            ...
            for (
                ...
                active_dev_count,
                throughput_per_dev,
            ) in throughput_per_active_dev_data:
    ...
    def generate_summary_report(
        ...
        summary = {
            ...
            "total_commits": sum(
                repo_metrics.get("total_commits", 0)
        ...
        for repo_name, repo_metrics in metrics.items():
            summary["total_unique_authors"].update(
                set(log._author[0] for log in git_log())
                if "logs" in locals()
                else set()
            )
            ...
            summary["repo_summaries"][repo_name] = {
                ...
                "avg_cycle_time": self._calculate_avg_cycle_time(
                    repo_metrics.get("cycle_time_data", [])
                ),
                "avg_failure_rate": self._calculate_avg_failure_rate(
                    repo_metrics.get("failure_rate_data", [])
                ),
            }
    ...
    def save_aggregated_metrics(
        self, metrics: Dict[str, Dict[str, Any]], output_dir: str = "multi_repo_metrics"
    ) -> str:
        """
        ...
        aggregated_throughput_per_active_dev = (
            self.aggregate_throughput_per_active_dev_metrics(metrics)
        )

        # Save aggregated data as CSV
        if aggregated_cycle_time:
            df_cycle = pd.DataFrame(
                aggregated_cycle_time, columns=["Month", "Sum", "Average", "P75", "Std"]
            )
            df_cycle.to_csv(
                os.path.join(output_dir, "aggregated_cycle_time.csv"), index=False
            )

        if aggregated_failure_rate:
            df_failure = pd.DataFrame(
                aggregated_failure_rate, columns=["Month", "Rate"]
            )
            df_failure.to_csv(
                os.path.join(output_dir, "aggregated_failure_rate.csv"), index=False
            )

        if aggregated_active_devs:
            df_devs = pd.DataFrame(
                aggregated_active_devs, columns=["Month", "ActiveDevelopers"]
            )
            df_devs.to_csv(
                os.path.join(output_dir, "aggregated_active_developers.csv"),
                index=False,
            )

        if aggregated_throughput:
            df_throughput = pd.DataFrame(
                aggregated_throughput, columns=["Month", "Commits"]
            )
            df_throughput.to_csv(
                os.path.join(output_dir, "aggregated_throughput.csv"), index=False
            )

        if aggregated_throughput_per_active_dev:
            df_throughput_per_dev = pd.DataFrame(
                aggregated_throughput_per_active_dev,
                columns=["Week", "ThroughputPerActiveDev"],
            )
            df_throughput_per_dev.to_csv(
                os.path.join(output_dir, "aggregated_throughput_per_active_dev.csv"),
                index=False,
            )
