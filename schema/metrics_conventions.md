...
Interchange DDL lives under `schema/metrics_*.sql`; each table has an ADR in `docs/adr/0002`–`0006`.
...
| **Week bucket** | `period_week` as ISO week label `YYYY-Www` from `%G` / `%V` in localtime ([ADR 0006](../docs/adr/0006-metrics-author-commit-weekly.md)). |
